-- Fix incomplete field updates in apply_selected_extraction_changes function
-- This migration ensures all detected changes are properly applied during updates

-- Update the apply_selected_extraction_changes function to handle all detected changes
...
-- Add comment explaining the comprehensive field coverage fix
COMMENT ON FUNCTION apply_selected_extraction_changes IS 'Applies selected extraction changes from a session. Updated in 20250723 to handle ALL detected field changes: variant, horsepower, fuel_type, transmission, body_type, year, wltp, co2_emission, co2_tax_half_year, consumption_l_100km, consumption_kwh_100km, seats, doors, mileage, and offers.';