-- Fix ambiguous column reference in apply_selected_extraction_changes function
-- This fixes the error: 'column reference "existing_listing_id" is ambiguous'
-- when processing DELETE changes
  ...
  LOOP
    ...
    ELSIF change_record.change_type = 'delete' AND existing_listing_id IS NOT NULL THEN
      BEGIN
        ...
        -- FIX: Qualify the column name to avoid ambiguity
        DELETE FROM extraction_listing_changes 
        WHERE extraction_listing_changes.existing_listing_id = change_record.existing_listing_id 
          AND extraction_listing_changes.id != change_record.id;
...
'Applies selected extraction changes from a session. Fixed ambiguous column reference in DELETE section where extraction_listing_changes.existing_listing_id was not properly qualified.';