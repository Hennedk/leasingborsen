-- Fix the ambiguity in the deletion logic - properly qualify column references
  ...
  LOOP
    ...
    ELSIF change_record.change_type = 'delete' AND existing_listing_id IS NOT NULL THEN
      BEGIN
        ...
        -- This includes changes from ALL sessions, not just the current one
        DELETE FROM extraction_listing_changes elc
        WHERE elc.existing_listing_id = v_listing_to_delete
          AND elc.change_status IN ('pending', 'discarded') -- Don't delete already applied/rejected changes
          AND elc.id != change_record.id; -- Don't delete the current record
