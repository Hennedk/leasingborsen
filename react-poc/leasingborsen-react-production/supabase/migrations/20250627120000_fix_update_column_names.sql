-- Fix column names in apply_extraction_session_changes function
-- The UPDATE section was using incorrect column names (fuel_type instead of fuel_type_id, etc.)
      ...
      BEGIN
        CASE v_change.change_type
          ...
          WHEN 'update' THEN
            ...
            -- Update existing listing with changed fields (FIXED COLUMN NAMES)
            UPDATE listings
            SET 
...
COMMENT ON FUNCTION apply_extraction_session_changes IS 'Applies all approved changes from an extraction session. FIXED: Column names corrected (fuel_type_id instead of fuel_type, etc.)';