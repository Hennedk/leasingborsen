-- Fix apply_selected_extraction_changes function with proper error handling
-- Uses valid change_status values and tracks errors in review_notes
...
COMMENT ON FUNCTION apply_selected_extraction_changes IS 'Apply selected changes from extraction session with proper error handling. Removes problematic exception handler inside loop.';