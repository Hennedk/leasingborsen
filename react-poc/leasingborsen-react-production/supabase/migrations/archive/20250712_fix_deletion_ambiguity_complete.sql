-- Fix ALL ambiguous column references in apply_selected_extraction_changes function
-- This completely fixes the DELETE section to properly qualify all column references
  ...
  LOOP
    ...
    ELSIF change_record.change_type = 'delete' AND existing_listing_id IS NOT NULL THEN
      BEGIN
        ...
        -- FIX: Properly qualify ALL column references to avoid ambiguity
        DELETE FROM extraction_listing_changes elc
        WHERE elc.existing_listing_id = change_record.existing_listing_id
          AND elc.id != change_record.id
          AND elc.change_status IN ('pending', 'discarded');
        ...
        IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'price_change_log') THEN
          DELETE FROM price_change_log WHERE listing_id = change_record.existing_listing_id;
        END IF;
        
        -- Delete lease pricing data
        DELETE FROM lease_pricing WHERE listing_id = change_record.existing_listing_id;
        
        -- Delete the listing
        DELETE FROM listings WHERE id = change_record.existing_listing_id;
...
COMMENT ON FUNCTION apply_selected_extraction_changes(UUID, UUID[], TEXT) IS 
'Applies selected extraction changes from a session. Fixed ambiguous column references in DELETE section by using table aliases and fully qualified column names.';