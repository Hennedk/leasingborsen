-- Final fix for apply_selected_extraction_changes function
-- Removes colour column that doesn't exist in listings table
...
BEGIN
  ...
  LOOP
    ...
    BEGIN  -- Begin block for each change
      ...
      IF change_record.change_type = 'create' THEN
        ...
        -- Insert new listing (removed colour)
      ...
      ELSIF change_record.change_type = 'update' AND existing_listing_id IS NOT NULL THEN
        ...
        -- Update existing listing (removed colour)
        UPDATE listings SET
    ...
    EXCEPTION 
      WHEN OTHERS THEN
        ...
        -- Update change with error in review notes (using valid status)
        UPDATE extraction_listing_changes
        SET 
          change_status = 'rejected',  -- Use rejected instead of error
  ...
  IF EXISTS (
    ...
    SET 
      status = CASE 
        ...
        WHEN error_count < total_processed THEN 'completed'  -- Still mark as completed even with some errors
        ELSE 'failed'
      END,
      applied_at = NOW(),
      applied_by = p_applied_by
  ...
  ELSE
    ...
    SET 
      status = CASE 
        WHEN error_count = 0 THEN 'completed'
        WHEN error_count < total_processed THEN 'completed'
        ELSE 'failed'
      END,
      applied_at = NOW()
    WHERE id = p_session_id;
...
COMMENT ON FUNCTION apply_selected_extraction_changes IS 'Apply selected changes from extraction session. Fixed: removed colour column, proper error handling.';