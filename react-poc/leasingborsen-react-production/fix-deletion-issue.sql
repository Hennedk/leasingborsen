-- Fix for deletion failure in apply_selected_extraction_changes
-- The issue is likely related to the lease score triggers interfering with deletion

-- First, let's modify the mark_lease_score_stale function to handle deletion gracefully
...
-- Now let's improve the apply_selected_extraction_changes function to have better error handling
-- and verification of actual deletions
...
BEGIN
  ...
  LOOP
    ...
    -- Process UPDATE changes - [UPDATE logic remains the same as before]
    ELSIF change_record.change_type = 'update' AND existing_listing_id IS NOT NULL THEN
      -- [UPDATE logic - keeping same as current version for brevity]
      BEGIN
...
COMMENT ON FUNCTION apply_selected_extraction_changes IS 'Applies selected extraction changes from a session. Fixed in 20250725 to handle lease score triggers properly and verify actual deletions.';

-- Add comment explaining the trigger fix
COMMENT ON FUNCTION mark_lease_score_stale IS 'Marks listings for lease score recalculation when retail price or lease pricing changes. Fixed in 20250725 to handle DELETE operations properly.';