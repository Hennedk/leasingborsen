-- Phase 1: Fix deletion issue in apply_selected_extraction_changes function
-- Focus: Transaction control + cleanup timing fix
...
BEGIN
  ...
  LOOP
    ...
    -- Process DELETE changes - FIXED CLEANUP TIMING
    ELSIF change_record.change_type = 'delete' AND existing_listing_id IS NOT NULL THEN
      BEGIN
        ...
        -- PHASE 1 FIX: Clean up references IMMEDIATELY, not in post-loop
        -- Delete extraction_listing_changes that reference this listing (except current session)
...
EXCEPTION
  WHEN OTHERS THEN
    -- PHASE 1 FIX: Proper transaction rollback on any error
