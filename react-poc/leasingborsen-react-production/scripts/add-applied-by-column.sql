-- Add applied_by column to extraction_listing_changes table
-- This column tracks who applied or reviewed the changes

ALTER TABLE extraction_listing_changes 
ADD COLUMN IF NOT EXISTS applied_by TEXT;

-- Add comment for documentation
COMMENT ON COLUMN extraction_listing_changes.applied_by IS 'User or system that applied or reviewed this change';

-- Update the function to handle the missing column gracefully
...
-- Add comment for documentation
COMMENT ON FUNCTION apply_selected_extraction_changes IS 'Apply only selected changes from an extraction session and mark others as discarded. Handles foreign key constraints when deleting listings and missing columns gracefully.';