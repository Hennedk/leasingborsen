-- Fix the apply_selected_extraction_changes function to match actual listings table structure
-- This version removes engine_info and other non-existent columns
...
COMMENT ON FUNCTION apply_selected_extraction_changes IS 'Apply only selected changes from an extraction session. Uses actual listings table structure without engine_info column.';