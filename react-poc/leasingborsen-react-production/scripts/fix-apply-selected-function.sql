-- Fix the apply_selected_extraction_changes function to handle foreign key constraints
-- This update adds deletion of related extraction_listing_changes records before deleting listings
...
COMMENT ON FUNCTION apply_selected_extraction_changes IS 'Apply only selected changes from an extraction session and mark others as discarded. Handles foreign key constraints when deleting listings.';