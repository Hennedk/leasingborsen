...
-- This version properly handles UPDATE operations where we need existing make_id for model lookups
  ...
  LOOP
    ...
    ELSIF change_record.change_type = 'update' AND existing_listing_id IS NOT NULL THEN
      ...
      UPDATE listings SET
        ...
        engine_info = CASE WHEN extraction_data ? 'engine_info' THEN extraction_data->>'engine_info' ELSE engine_info END,
...
COMMENT ON FUNCTION apply_selected_extraction_changes IS 'Apply only selected changes from an extraction session. Properly handles ID lookups for reference tables with better error handling for UPDATE operations.';