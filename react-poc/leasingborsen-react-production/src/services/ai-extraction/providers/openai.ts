...
/**
 * OpenAI provider for extracting structured data from Danish car leasing documents
 * Uses GPT-4 with specialized prompts for Danish text and car leasing terminology
 */
export class OpenAIProvider extends BaseAIProvider {
  ...
  constructor() {
    ...
    this.modelVersion = config.openaiModel || 'gpt-4-turbo-preview'
  }
  ...
  private async initializeClient(): Promise<OpenAI> {
    if (!this.client) {
      ...
      this.client = new OpenAI({
        ...
        maxRetries: 1 // We handle retries at the service level
      })
  ...
  async extract(content: string, options: ExtractOptions = {}): Promise<ExtractionResult> {
    ...
    try {
      ...
      for (let attempt = 0; attempt <= maxRetries; attempt++) {
        try {
          const response = await client.chat.completions.create({
            ...
            max_tokens: 4000, // Allow for comprehensive structured output
          ...
          const mappedError = this.mapError(error, 'OpenAI API request')
      ...
      const finalError = this.mapError(lastError, 'OpenAI API request after all retries')
      const costCents = tokensUsed > 0 ? this.calculateCost(tokensUsed) : 0
      ...
      const mappedError = this.mapError(error, 'OpenAI extraction')
      const costCents = tokensUsed > 0 ? this.calculateCost(tokensUsed) : 0
   ...
   * Current pricing (as of 2024): $0.03/1K input tokens, $0.06/1K output tokens
   */
  calculateCost(tokens: number): number {
    ...
    // GPT-4 pricing in USD per 1K tokens
  ...
  private buildSystemPrompt(_options: ExtractOptions): string {
    return `You are an expert AI system for extracting structured data from Danish car leasing documents. 
...
Return the data as valid JSON matching the required structure.`
  ...
  /**
   * Override token estimation for OpenAI's tokenization
   * GPT-4 uses approximately 3-4 characters per token for Danish text
   */
  protected estimateTokens(content: string): number {
    if (!content) return 0
    
    // More accurate estimation for Danish text with GPT-4
    // Danish text tends to be more efficient than English due to compound words
    ...
    const promptOverhead = 600 // System prompt tokens
